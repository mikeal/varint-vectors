import { decode, encode, encoding_length, parse, parse_vector, encode_vector } from './index.js'
import test from 'tape'

const encodingLength = encoding_length


test('vectors', assert => {
  const fixture = [1, 129322, 12931023192 ]
  const encoded = encode_vector(fixture)
  const result  = parse_vector(encoded, fixture.length)
  const offset  = result.pop()
  assert.equal(offset, encoded.length)
  assert.same(result, fixture)
  assert.end()
})
